...
export default function Sidebar({
    ...
    return (
      <aside
        ...
        className={`fixed left-0 top-16 z-40 h-[calc(100vh-64px)] w-72 transform border-r border-(--quaternary-color)/10 bg-linear-to-b from-(--bg-secondary)/95 to-(--sixth-color)/95 backdrop-blur-sm transition-transform duration-300 ease-in-out ${
          isOpen
            ? "translate-x-0 overflow-visible pointer-events-auto"
  ...
  return (
    <aside
      className={`fixed left-0 top-16 z-30 h-[calc(100vh-64px)] shrink-0 border-r border-(--quaternary-color)/10 bg-linear-to-b from-(--bg-secondary)/92 to-(--sixth-color)/92 transition-[width] duration-300 ease-in-out ${
        isOpen ? "w-72 overflow-visible pointer-events-auto" : "w-0 overflow-hidden pointer-events-none"
      }`}
    >
      <div className="flex h-full flex-col">
        <div className="flex-1">
