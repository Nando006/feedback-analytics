...
export default function AnalyticsPositiveSummarySection({
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-(--quaternary-color)/10 bg-gradient-to-br from-(--bg-secondary) to-(--sixth-color) p-6 glass-card">
      ...
      <p className="mb-4 text-sm font-work-sans text-[var(--text-tertiary)]">
        ...
        <div className="space-y-2 md:col-span-2">
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            {summary.topCategories.slice(0, 5).map((cat) => (
              <span
                key={cat.name}
                className="rounded-full border border-(--positive)/40 bg-(--positive)/10 px-3 py-1 text-(--positive) font-work-sans">
                {cat.name} ({cat.count})
              </span>
            ))}
