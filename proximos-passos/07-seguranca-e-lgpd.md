...
- **Information disclosure / minimização:** o trigger `clean_user_metadata_before_change` (`database/sql/functions/public/clean_user_metadata_before_change__0aa1f366fd.sql`) remove do `raw_user_meta_data` as chaves `phone`, `document`, `company_name`, `account_type`, `terms_version`, `terms_accepted_at`, `email` — logo, esses dados **não entram no JWT**. O JWT só carrega claims seguros via `jwt_custom_claims` (`role` + `enterprise_id`). Documentado em [`docs/implementacao/funcionalidades/higienizacao-jwt-lgpd.md`](../docs/implementacao/funcionalidades/higienizacao-jwt-lgpd.md).
...
- [Higienização de dados sensíveis no JWT (LGPD)](../docs/implementacao/funcionalidades/higienizacao-jwt-lgpd.md) — mecanismo de minimização já implementado
